...
$(function(){
	...
	var height = 0;
	var op = 0, cp = 0;
	var vdi_style = $("#vdi_style")[0].sheet;
	var rules = vdi_style.rules;
	$(".progress_resize_handler")
	console.log($(".progress_resize_handler"));
	var masker = $("<div id=PageMasker></div>");
	masker.appendTo(document.body).mousemove(function(e){
		//console.log(enable);
		if(enable){
			rules[2].style.paddingBottom =
			rules[1].style.height = Math.max(40, Math.min(200, height + (op - e.clientY))) + "px";
		}
	}).mouseup(function(e){
		enable = false;
		console.log("!!!");
	...
	$(document).mousedown(function(e){
		if(e.target.classList.contains("progress_resize_handler")){
			enable = true;
			masker.addClass("show");
			op = e.clientY;
			height = parseInt(rules[1].style.height);
			console.log(height, rules[1]);
		}
	})
});
